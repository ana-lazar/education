...
public class ParticipantJdbcRepository extends AbstractJdbcRepository<Integer, Participant> implements ParticipantRepository {
    ...
    public List<Participant> findByTeam(Integer teamId) {
        System.out.println("find participants with team " + teamId);
        if (teamId == null) {
            System.out.println("find failed: team is null");
        ...
        if (participants.isEmpty()) {
            System.out.println("find failed: team {} not found" + teamId);
            return participants;
        }
        System.out.println("find successful " + participants.get(0));
        return participants;
    }

    @Override
    public List<Participant> findByRace(Integer raceId) {
        System.out.println("find participants with race " + raceId);
        if (raceId == null) {
            System.out.println("find failed: race is null");
        ...
        if (participants.isEmpty()) {
            System.out.println("find failed: race {} not found" + raceId);
            return participants;
        }
        System.out.println("find successful " + participants.get(0));
        return participants;
