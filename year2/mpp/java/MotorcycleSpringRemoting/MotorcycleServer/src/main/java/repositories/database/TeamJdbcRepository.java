...
public class TeamJdbcRepository extends AbstractJdbcRepository<Integer, Team> implements TeamRepository {
    ...
    public Integer findIdByName(String name) {
        System.out.println("find team with name " + name);
        if (name == null) {
            System.out.println("find failed: name is null");
        ...
        if (teams.isEmpty()) {
            System.out.println("find failed: name {} not found" + name);
            return 0;
        }
        System.out.println("find successful " + teams.get(0));
