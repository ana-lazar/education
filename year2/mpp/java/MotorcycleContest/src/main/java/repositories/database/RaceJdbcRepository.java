...
public class RaceJdbcRepository extends AbstractJdbcRepository<Integer, Race> implements RaceRepository {
    ...
    public Integer findIdByCapacity(Integer capacity) {
        LOGGER.traceEntry("find race with capacity {} ", capacity);
        if (capacity == null) {
            LOGGER.warn("find failed: capacity is null");
        ...
        if (races.isEmpty()) {
            LOGGER.warn("find failed: capacity {} not found", capacity);
            return -1;
        }
        LOGGER.traceExit("find successful {}", races);
        return races.get(0).getId();
    }

    @Override
    public List<Race> findByCapacityInterval(Integer capacity1, Integer capacity2) {
        LOGGER.traceEntry("find race with capacity between {} and {} ", capacity1, capacity2);
        if (capacity1 == null || capacity2 == null) {
            LOGGER.warn("find failed: capacities are null");
        ...
        if (races.isEmpty()) {
            LOGGER.warn("find failed: capacity between {} and {} not found", capacity1, capacity2);
            return races;
        }
        LOGGER.traceExit("find successful {}", races);
        return races;
